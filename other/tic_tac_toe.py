...
A move is guaranteed to be valid, and a valid move is one placed on an
empty block in the grid. A player who succeeds in placing n of their marks
in a horizontal, diagonal, or vertical row wins the game. Once a winning
condition is reached, the game ends and no more moves are allowed. Below
is an example game which ends in a winning condition:
...
class TicTacToe(object):
    ...
    def move(self, row, col, player):
        if 0 > row >= self.gridSize:
            print('Bad row index')
            return None
        
        if 0 > col >= self.gridSize:
            print('Bad column index')
            return None
        
        if self.grid[row][col] != 0:
            print('cell ['+str(row)+','+str(col)+'] already used')
            return None
        ...
        if (row+col+1) == self.gridSize:
            for i in range(self.gridSize):
                if self.grid[(i+1)*(-1)][i] != player:
                        return False
